...
xhr.open('GET', 'https://stats.cryptohands.org/info');
xhr.send();
xhr.onload = function() {
  if (xhr.status != 200) { 
    ...
    var usdtPrice = obj.data.ethToUsdt;
                ...
                var date1 = new Date(2019,3,20);
                var date2 = new Date();
                var daysLag = Math.ceil(Math.abs( (date2.getTime() - date1.getTime()) * 100) / (1000 * 3600 * 24 * 3650))/100;
                //console.log(daysLag);
